...
"""PeriFlow Mistral Checkpoint Converter."""
...
class MistralForCausalLMConverter(LlamaForCausalLMConverter):
    ...
    def check_config(self) -> None:
        """Check if Mistral architectures' config can be converted to PeriFlow format."""
        super().check_config()
        try:
            ...
            if cast(MistralConfig, self.config).rope_theta != 10000.0:
                raise NotSupportedCheckpointError(
                    invalid_option=f"'rope_theta={cast(MistralConfig, self.config).rope_theta}'",
                    valid_options=[10000.0],
                )
