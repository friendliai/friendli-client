...
from periflow.modules.converter.utils import get_tensor_from_state_dict
...
class ConversionInterface(ABC):
    ...
    def convert_non_transformer_layers(
        ...
        for converted_layer_name, (
            ...
            converted_np_array = convert_fn(state_dict, "", per_layer_postfixes)
    ...
    def convert_decoder_layers(
        ...
        for i in range(self.decoder_layer_num):
            ...
            for converted_layer_name, (
                ...
                converted_np_array = convert_fn(state_dict, layer, per_layer_postfixes)
...
class QuantConversionInterface(ABC):
    ...
    def quantized_convert_dict(self) -> Dict[str, Any]:
        ...
        return {
            "attn/c_attn/q_weight_scale:0": (self.scale_convert, [".q.weight_scale"]),
            "attn/c_attn/k_weight_scale:0": (self.scale_convert, [".k.weight_scale"]),
            "attn/c_attn/v_weight_scale:0": (self.scale_convert, [".v.weight_scale"]),
            "attn/c_attn/q_out_scale:0": (self.scale_convert, [".q.out_scale"]),
            "attn/c_attn/k_out_scale:0": (self.scale_convert, [".k.out_scale"]),
            "attn/c_attn/v_out_scale:0": (self.scale_convert, [".v.out_scale"]),
            "attn/c_attn/in_scale:0": (self.scale_convert, [".q.in_scale"]),
            "attn/c_proj/weight_scale:0": (
                self.scale_convert,
                [".attn_fc.weight_scale"],
            ),
            "attn/c_proj/out_scale:0": (self.scale_convert, [".attn_fc.out_scale"]),
            "attn/c_proj/in_scale:0": (self.scale_convert, [".attn_fc.in_scale"]),
            "mlp/c_fc/weight_scale:0": (self.scale_convert, [".ff1.weight_scale"]),
            "mlp/c_fc/out_scale:0": (
                self.scale_convert,
                [".ff1.out_scale"],
            ),
            "mlp/c_fc/in_scale:0": (
            ...
            "mlp/c_proj/weight_scale:0": (
                self.scale_convert,
                [".ff2.weight_scale"],
            ),
            "mlp/c_proj/out_scale:0": (
                self.scale_convert,
                [".ff2.out_scale"],
            ),
            "mlp/c_proj/in_scale:0": (
                self.scale_convert,
                [".ff2.in_scale"],
            ),
            "attn/c_attn/int8_weight:0": (
            ...
            "attn/c_proj/int8_weight:0": (
                self.quantized_linear_weight_convert,
                [".attn_fc.int8_weight"],
            ),
            "mlp/c_fc/int8_weight:0": (
                self.quantized_linear_weight_convert,
                [".ff1.int8_weight"],
            ),
            "mlp/c_proj/int8_weight:0": (
                self.quantized_linear_weight_convert,
                [".ff2.int8_weight"],
            ),
        }
    ...
    def convert_quantized_layers(
        ...
        for i in range(self.quantized_layer_num):
            ...
            for converted_layer_name, (
                convert_fn,
                per_layer_postfixes,
            ) in self.quantized_convert_dict.items():
                converted_np_array = convert_fn(  # pylint: disable=not-callable
                    state_dict, layer, per_layer_postfixes
                )
                per_layer_out_ckpt[converted_layer_name] = converted_np_array
                pbar.update()
