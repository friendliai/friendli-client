...
from periflow.modules.converter.utils import get_tokenizer, save_tokenizer
from periflow.modules.quantizer.configurator import SmoothQuantConfigurator
from periflow.modules.quantizer.maps import (
    check_support_smoothquant,
    get_smoothquant_quantizer,
...
def convert_checkpoint(  # pylint: disable=too-many-locals
    ...
    *,
    ...
    smoothquant_config: Optional[Dict[str, Any]] = None,
) -> None:
    ...
    Args:
        ...
        smoothquant (bool, optional): Enable SmoothQuant. Defaults to False.
        smoothquant_config (Optional[Dict[str, Any]], optional): SmoothQuant configuration.
            Defaults to None.
    ...
    if smoothquant and smoothquant_config:
        smoothquant_configurator = SmoothQuantConfigurator(config=smoothquant_config)
        smoothquant_configurator.validate()
        validated_smoothquant_config: Dict[str, Any] = smoothquant_config[
            "smoothquant_config"
        ]

        if not check_support_smoothquant(model_arch):
            raise NotSupportedCheckpointError(
                invalid_option=f"'smoothquant={smoothquant}', but 'architectures={model_arch}'",
                valid_options=list(model_arch_smoothquant_hook_map.keys()),
            )
        smoothquant_quantizer = get_smoothquant_quantizer(
            model_arch, model_config, validated_smoothquant_config
        )
        smoothquant_quantizer.check_config()
    ...
    if not dry_run:
        ...
        if smoothquant:
            ...
            smoothquant_quantizer.pre_quantize(model, tokenizer, data_type)
            quant_result = smoothquant_quantizer.quantize(model, tokenizer, data_type)
            state_dict = model.state_dict()
            state_dict.update(get_quantized_state_dict(quant_result))
        else:
            state_dict = model.state_dict()
